...
export class AboutService {
  ...
  async getAboutInfo(clientIp: string, baseUrl: string) {
    const getServices = async () => {
      ...
      return result.services.map((service) => {
        let oauth_url = service.oauth_url;
        if (oauth_url && oauth_url.startsWith('/')) {
          oauth_url = `${baseUrl}${oauth_url}`;
        }
        return {
          name: service.name,
    ...
    const servicesWithDetails = await Promise.all(
      services.map(async (serviceName) => {
        ...
        let oauth_url = serviceName.oauth_url;
        if (oauth_url && oauth_url.startsWith('/')) {
          oauth_url = `${baseUrl}${oauth_url}`;
        }
